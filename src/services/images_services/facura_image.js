...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    const uploadDir = path.join(__dirname, "../../upload/factura_images");
    ...
    fs.readdir(uploadDir)
      .then((files) => {
        const deletePromises = files
          .filter(
            (existingFile) =>
              existingFile.startsWith(baseName) && existingFile !== filename
          )
...
router.post(
  ...
  (req, res) => {
    try {
      ...
      const baseUrl =
        req.protocol + "://" + req.get("host") + "/uploads/factura_images/";
